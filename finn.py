...
def main():
    ...
    # Rest of your existing app functionality for authenticated users
    if st.session_state.authenticated:
        if st.session_state.access_level == "full":  # Admin
            ...
            run_business_app()
            # Add business team-specific functionality here
...
def show_login_page():
    ...
    with col2:
        st.subheader("Legato Team")
        if st.button("Login as Legato"):
            st.session_state.login_type = "legato"
            st.rerun()
    
    with col3:
        st.subheader("Business Team")
        if st.button("Login as Business"):
            st.session_state.login_type = "business"
            st.rerun()
    
    if hasattr(st.session_state, 'login_type'):
        st.markdown("---")
        with st.form("login_form"):
            ...
            if submit:
                ...
                if result["authenticated"]:
                    ...
                    if (username.lower() == st.session_state.login_type or 
                        (username.lower() == "admin" and st.session_state.login_type == "admin")):
                        st.session_state.authenticated = True
                        st.session_state.current_user = username
                        st.session_state.access_level = result["access_level"]
                        st.rerun()
                    else:
                        st.error("You don't have permission to access this role")
                else:
                    st.error("Invalid credentials")

def run_admin_app():
    # Your existing app code goes here - everything that was in main() before
    # Keep all the existing functionality for admin
...
def run_legato_app():
    st.title("📊 Legato Team - Shipment Overview")
    ...
    with st.container(border=True):        
        ...
        if analysis_option == "Fetch from Google Sheets":
            ...
            if (st.session_state.expo_processed or st.session_state.maersk_processed or 
                ...
                st.warning("No processed data available - please ask admin to process files first")
        ...
        if not combined.empty:
            ...
            # Filter block - Updated with CUSDEC filters
            st.markdown("### 🔎 Filter Records")
            filter_col1, filter_col2 = st.columns(2)
            ...
            with filter_col1:
                ...
                # Add CUSDEC No filter
                if 'CUSDEC No' in combined.columns:
                    selected_cusdec_nos = st.multiselect(
                        "Filter by CUSDEC No",
                        options=combined['CUSDEC No'].dropna().unique(),
                        default=None
                    )
            ...
            with filter_col2:
                ...
                # Add CUSDEC Date filter
                if 'CUSDEC Date' in combined.columns:
                    selected_cusdec_dates = st.multiselect(
                        "Filter by CUSDEC Date",
                        options=combined['CUSDEC Date'].dropna().unique(),
                        default=None
                    )
            ...
            if selected_invs:
                filtered_combined = filtered_combined[filtered_combined['Inv #'].isin(selected_invs)]
            if 'CUSDEC No' in combined.columns and 'selected_cusdec_nos' in locals() and selected_cusdec_nos:
                filtered_combined = filtered_combined[filtered_combined['CUSDEC No'].isin(selected_cusdec_nos)]
            if 'CUSDEC Date' in combined.columns and 'selected_cusdec_dates' in locals() and selected_cusdec_dates:
                filtered_combined = filtered_combined[filtered_combined['CUSDEC Date'].isin(selected_cusdec_dates)]
            ...
            with col1:
                ...
                st.download_button(
                    ...
                    data=csv,
                    file_name='legato_filtered_data.csv',
                    mime='text/csv'
                )
...
def run_business_app():
    ...
    with st.container(border=True):        
        ...
        if not combined.empty:
            ...
            # Filter block - Business Team specific filters
            st.markdown("### 🔎 Filter Records")
            filter_col1, filter_col2 = st.columns(2)
            ...
            if 'PO #' in combined.columns and 'selected_pos' in locals() and selected_pos:
                filtered_combined = filtered_combined[filtered_combined['PO #'].isin(selected_pos)]
            if 'ETA' in combined.columns and 'selected_eta_dates' in locals() and selected_eta_dates:
                filtered_combined = filtered_combined[filtered_combined['ETA'].dt.date.isin(selected_eta_dates)]
            if 'Origin Vessel' in combined.columns and 'selected_vessels' in locals() and selected_vessels:
                filtered_combined = filtered_combined[filtered_combined['Origin Vessel'].isin(selected_vessels)]
            ...
            # Apply delivery status highlighting without showing status column
            if 'Delivery date' in filtered_combined.columns:
                try:
                    # Create display dataframe without sheet and status columns
                    columns_to_show = [col for col in filtered_combined.columns 
                                     if col not in ['sheet', 'Delivery Status']]
                    display_df = filtered_combined[columns_to_show].copy()
                    
                    # Apply color formatting
                    def color_delivery_status(row):
                        if pd.notna(filtered_combined.loc[row.name, 'Delivery date']):
                            return ['background-color: lightgreen'] * len(row)
                        return [''] * len(row)
                    
                    # Show filtered data with delivery status highlighting
                    with st.expander("🔍 View Filtered Data", expanded=True):
                        st.dataframe(
                            display_df.style.apply(color_delivery_status, axis=1),
                            use_container_width=True,
                            height=400
                        )
                    ...
                    delivery_counts = {
                        'Delivered': filtered_combined['Delivery date'].notna().sum(),
                        'Pending': len(filtered_combined) - filtered_combined['Delivery date'].notna().sum()
                    }
                    ...
                    with st.expander("🔍 View Filtered Data", expanded=True):
                        st.dataframe(
                            filtered_combined[[col for col in filtered_combined.columns if col != 'sheet']],
                            use_container_width=True,
                            height=400
                        )
            else:
            ...
            st.download_button(
                label="Download Filtered Data",
                data=csv,
                file_name='business_filtered_data.csv',
                mime='text/csv'
            )
